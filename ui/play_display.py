    ...
    while(i < 4):
        ...
        while(h < 3):
            if(h == 1):
                ...
                li = colored(' ', 'grey', 'on_white')  # Initialisation ligne secondaire
                while(j < 4): 
                    indice = (i*4)+j  # Calcul indice
                    li += colored(sized(plateau['tiles'][indice]), 'grey', get_color(plateau['tiles'][indice]))+colored(' ', 'grey', 'on_white')  # Concaténation texte coloré
                ...
                while(j < 4):
                    indice = (i*4)+j  # Calcul indice
                    li += colored('     ', 'grey', get_color(plateau['tiles'][indice]))+colored(' ', 'grey', 'on_white')  # Concaténation texte coloré
                    j += 1  # Incrémentation j
        ...
        print(colored('                         ', 'grey', 'on_white'))  # Permet de séparer les lignes principales (sans couleurs)
